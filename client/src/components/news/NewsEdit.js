...
import { getNewsDetail, updateNews, getNewsCategories, uploadNewsMedia } from '../../service/newsApi';
...
export default function NewsEdit() {
  ...
  useEffect(() => {
    // 기존 스타일이 있다면 제거
    const existingStyle = document.getElementById('blocknote-image-styles-edit');
    if (existingStyle) {
      existingStyle.remove();
    }
    
    const styleElement = document.createElement('style');
    styleElement.id = 'blocknote-image-styles-edit';
    styleElement.textContent = imageResizeStyles;
    document.head.appendChild(styleElement);
    
    return () => {
      const styleToRemove = document.getElementById('blocknote-image-styles-edit');
      if (styleToRemove) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 에디터에 기존 내용 설정
        if (newsData.content) {
          editor.replaceBlocks(editor.document, JSON.parse(newsData.content));
        }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">뉴스 수정</h1>
        <p className="text-gray-600">기존 뉴스 내용을 수정합니다.</p>
      </div>
