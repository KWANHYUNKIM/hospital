...
export default function RecommendedChannels() {
  ...
  useEffect(() => {
    const fetchChannels = async () => {
      try {
        ...
        setChannels(response.filter(item => 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await getSocialChannels();
      setChannels(response.filter(item => item.type === 'CHANNEL'));
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-bold">추천 채널</h2>
        {isAdmin && (
          <button
          ...
          <div key={channel.id} className="relative group">
            ...
            >
              <img
                ...
                alt={channel.name}
                className="w-14 h-14 rounded-full border border-gray-200 object-cover bg-white shadow-sm group-hover:shadow-md transition-shadow duration-200"
              />
              <div className="text-sm text-center mt-1 group-hover:text-blue-500 transition-colors duration-200">
                {channel.name}
              </div>
            </a>
