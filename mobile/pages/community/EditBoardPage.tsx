...
const EditBoardPage: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [boardResponse, categoriesResponse] = await Promise.all([
          ...
          axios.get('http://localhost:8080/api/boards/categories', { withCredentials: true })
        ]);

        const board: Board = boardResponse.data;
        setTitle(board.title);
        setContent(board.summary);
        setSelectedCategory(board.categoryId);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axios.put(`http://localhost:8080/api/boards/${id}`, requestData, {
      ...
      Alert.alert('성공', '게시글이 수정되었습니다.', [
        {
          text: '확인',
          onPress: () => {
            console.log('게시글 상세 페이지로 이동:', id);
            // navigation.navigate('BoardDetail', { id: id });
          }
        }
      ]);
    } catch (error) {
